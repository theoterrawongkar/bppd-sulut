<x-main-layout>
    ...
    <section class="container mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <form action="{{ route('myculinaryplace.index') }}" method="GET">
            {{-- Heading & Search --}}
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
                ...
                <div class="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
                    ...
                    {{-- Tombol Tambah --}}
                    <a href="{{ route('myculinaryplace.create') }}"
                            ...
                            <div class="flex flex-col sm:flex-row">
                                ...
                                <div class="w-full p-4 flex flex-col justify-between space-y-3">
                                    <div>
                                        ...
                                        {{-- Deskripsi & Kontak --}}
                                        <p class="text-sm text-gray-600 line-clamp-3">{{ $culinaryPlace->description }}
                                        </p>
                                        <div class="mt-2 space-y-1">
                                            ...
                                            @endif
                                            <p class="flex items-center gap-2 text-sm text-[#486284]">
                                                <x-icons.location class="shrink-0" /> {{ $culinaryPlace->address }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between mt-auto">
                                        <div class="text-sm text-gray-400">
                                            Dibuat pada {{ $culinaryPlace->created_at->translatedFormat('d F Y') }}
                                        </div>

                                        <div class="flex items-center gap-3">
                                            <a href="{{ route('myculinaryplace.edit', $culinaryPlace->slug) }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.querySelector('form[action="{{ route('myculinaryplace.index') }}"]');
