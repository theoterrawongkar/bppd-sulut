<x-app-layout>
    ...
    <section>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-4">
            {{-- Judul --}}
            <h1 class="text-xl font-bold">Manajemen Seniman</h1>

            {{-- Form Pencarian dan Status --}}
            <form id="searchArtistForm" action="{{ route('dashboard.artistprofile.index') }}" method="GET"
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                    ...
                    placeholder="Cari nama seniman..." autocomplete="off">
                ...
                <select name="status"
                    ...
                    <option value="Berhenti Permanen" {{ request('status') == 'Berhenti Permanen' ? 'selected' : '' }}>
                        Berhenti Permanen</option>
                </select>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-[#F4F8FC]">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">Nama Panggung</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">Email Penanggung Jawab</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">Bidang</th>
                ...
                <tbody class="divide-y divide-gray-200 text-sm">
                    @forelse ($artistProfiles as $artistProfile)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-2">{{ $loop->iteration }}</td>
                            <td class="px-4 py-2 font-semibold">{{ $artistProfile->stage_name }}</td>
                            <td class="px-4 py-2">{{ Str::limit($artistProfile->owner_email, 20, '...') }}</td>
                            <td class="px-4 py-2">{{ $artistProfile->field ?? '-' }}</td>
                            <td class="px-4 py-2 text-center whitespace-nowrap">
                                ...
                                @php
                                    ...
                                    $badgeColor = match ($status) {
                                        ...
                                        'Berhenti Permanen' => 'bg-gray-200 border border-gray-400 text-gray-800',
                            ...
                            <td class="px-4 py-2 text-center whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($artistProfile->created_at)->format('d M Y') }}
                            </td>
                            <td class="px-4 py-2 space-x-2 text-center whitespace-nowrap">
                                <a href="{{ route('dashboard.artistprofile.edit', $artistProfile->id) }}"
                                    class="text-yellow-600 hover:underline">Ubah</a>
                                <form action="{{ route('dashboard.artistprofile.destroy', $artistProfile->id) }}"
                                    method="POST" class="inline-block"
                                    onsubmit="return confirm('Yakin ingin menghapus seniman ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center px-4 py-4 text-gray-500">Belum ada data seniman.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        {{-- Paginator --}}
        <div class="mt-5">
            {{ $artistProfiles->links('pagination::app') }}
        </div>
    </section>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('searchArtistForm');
            if (!form) return;
